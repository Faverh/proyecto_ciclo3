...
def activate():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                #'SELECT * FROM activationlink WHERE number = ?', (number, utils.U_UNCONFIRMED)
            ).fetchone()

            if attempt is not None:
                db.execute(
                    #'UPDATE activationlink SET ', (utils.U_CONFIRMED, attempt['id'])
                )
                db.execute(
                    'INSERT INTO username (username, password, salt, email VALUES (?,?,?,?))',
...
def register():
    try:
        ...
        if request.method == 'POST':    
            ...
            db.execute(
                'INSERT TO activationlink (number, utils.U_UNCONFIRMED, username, hashP, salt, email VALUES (?,?,?,?))',
...
def change():
    try:
        ...
        if request.method == 'GET': 
            ...
            db = get_db()
            attempt = db.execute(
                #'SELECT * FROM forgotlink', (number, utils.F_ACTIVE)
            ).fetchone()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if user is not None:
                ...
                db.execute(
                    'INSERT INTO forgotlink (user[id], number, utils.F_ACTIVE VALUES (?,?,?))',
